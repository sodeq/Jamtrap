...
RGB Camera::at(const IplImage& m,int x, int y) {
	RGB out;
	out.b = m.imageData[3*(y*m.width+x) + 0];
	out.g = m.imageData[3*(y*m.width+x) + 1];
	out.r = m.imageData[3*(y*m.width+x) + 2];
	return out;
}
#endif
